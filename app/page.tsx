...
export default function Home() {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      const { parseExcelFile, aggregateData } = await import("@/lib/dataProcessor");
      const raw = await parseExcelFile(file);
      
      const anomalousRows = raw.filter(row => {
      ...
      if (anomalousRows.length > 0) {
          ...
          const result = aggregateData(raw);
          setData(result);
      }
    } catch (err) {
  ...
  const handleFixedData = async (fixedData: any[]) => {
      const { aggregateData } = await import("@/lib/dataProcessor");
      const result = aggregateData(fixedData);
      setData(result);
      setNeedsFixing(false);
      setRawData(null);
  };
          ...
          <div className={`flex-grow transition-all duration-300 ${showHistory ? 'lg:w-3/4' : 'w-full'}`}>
            ...
            {needsFixing && rawData && (
              <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 mt-8">
                 <DataFixer rawData={rawData} onFixed={handleFixedData} />
              </div>
            )}
